...
class SplitWord
{
	...
	var $CommonUnit = "年|月|日|时|分|秒|点|元|百|千|万|亿|位|辆|口|半|个";
	
	var $CnNumber = "％|＋|－|０|１|２|３|４|５|６|７|８|９|．";
	var $CnSgNum = "一|二|三|四|五|六|七|八|九|十|百|千|万|亿|数|俩";
	var $MaxLen = 20; //词典最大 7 中文字，这里的数值为字节数组的最大索引
	var $MinLen = 5;  //最小 2 中文字，这里的数值为字节数组的最大索引
	...
	{  	  	
		//$this->cacheDict();die();
		$file = __PUBLIC__."dict/dict.cache";  
		$handle = fopen($file, "r");  
		$dict = unserialize(fread($handle, filesize ($file)));
		...
		$this->OneNameDic = $dict['oneNameDic'];
		$this->TwoNameDic = $dict['twoNameDic'];
		$this->RankDic = $dict['rankDic'];
	}

	function cacheDict()
	{  	  	
		//载入姓氏词典
		for($i=0;$i<strlen($this->CnOneName)-1;$i=$i+3)
		{
			$top = substr($this->CnOneName, $i, 3);
			$this->OneNameDic[$top] = 1;
		}
		...
		//高级分词，预先载入词典以提高分词速度
		$dicfile = __PUBLIC__."dict/dict.txt"; 
		$fp = fopen($dicfile,'r');
		...
		$dict = array(
			"rankDic" 		=> $this->RankDic,
			"oneNameDic" 	=> $this->OneNameDic,
			"twoNameDic" 	=> $this->TwoNameDic
		);
		$file = __PUBLIC__."dict/dict.cache"; 
		if(is_file($file))
		{
			file_put_contents($file, serialize($dict));
			echo "词典写入成功！";
		}
	}
		...
		*/
		...
		for($i=($spLen-1);$i>=0;$i--)
		{
			...
			else
			{
				...
				//标点符号
				else if(in_array($c, $this->biao_dian))
				{ 
					$this->ResultString = $spwords[$i].$spc.$this->ResultString; 
				}
				else //正常短句
				{
					...
					{ 
						...
						//$this->ResultString = $this->RunMM($spwords[$i]).$spc.$this->ResultString;
					}
				}
			}
		}
	...
	function RunMM($str)
	{
		...
		{
			//当i达到最小可能词的时候
			if($i>=($spLen-$this->MinLen))
			{
				if($i==$spLen-3)
				{
					$WordArray[] = substr($str,$spLen-3,3);
				}
				else
				{
					$start = $spLen-$this->MinLen-1;
					$w = substr($str, $start, $this->MinLen+1);
					if($this->IsWord($w))
					{
						$WordArray[] = $w;                                                                                  
					}
					...
					{
						$WordArray[] = substr($str, $start, 3);
						$WordArray[] = substr($str, $start+3, 3);
					}
				}
				$i = -1; break;
			}
			//分析在最小词以上时的情况
			if($i<=$spLen-$this->MaxLen-1)
			{
				$minPos = $this->MaxLen;
			}
			else 
	...
	function ParOther($WordArray)
	{
		...
		for($i=$wlen;$i>=0;$i--)
		{
			...
			//双字姓
			if(strlen($WordArray[$i])==6 && isset($this->TwoNameDic[$WordArray[$i]]))
			{
				$rsStr .= $spc.$WordArray[$i];
				if($i>0 && strlen($WordArray[$i-1])==3)
				{
					$rsStr .= $WordArray[$i-1];$i--;
					if($i>0 && strlen($WordArray[$i-1])==3)
					{ 
						if(ereg("^".$this->EspecialChar,$WordArray[$i-1]))
							continue;
						$rsStr .= $WordArray[$i-1];$i--;
					}
				}
			...
			else if(strlen($WordArray[$i])==3 && isset($this->OneNameDic[$WordArray[$i]]))
			{
				$rsStr .= $spc.$WordArray[$i];
				if($i>0 && strlen($WordArray[$i-1])==3)
				{
